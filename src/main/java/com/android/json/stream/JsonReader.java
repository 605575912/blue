...
public final class JsonReader implements Closeable {
    ...
    private void expect(JsonToken expected) throws IOException {
        peek();
        if (token == JsonToken.END_DOCUMENT && (expected == JsonToken.END_ARRAY || expected == JsonToken.END_OBJECT)){
            return ; // 2013.5.21 :容错处理，到文件结尾时，不抛出异常
        }
        if (token != expected) {
    ...
    public boolean hasNext() throws IOException {
        ...
        //2013.5.31:容错性处理，遇到token=JsonToken.END_DOCUMENT时返回 false
    }
            ...
            case EMPTY_DOCUMENT:
                ...
                if (token != JsonToken.BEGIN_ARRAY && token != JsonToken.BEGIN_OBJECT) {
                    throw new IOException(
                            "Expected JSON document to start with '[' or '{' but was " + token +",name="+name);
                }
                return firstToken;
            case EMPTY_ARRAY:
        ...
        try {
            ...
            do {
                ...
                if (token == JsonToken.BEGIN_ARRAY || token == JsonToken.BEGIN_OBJECT) {
                    ...
                    count--;
                } else if (token == JsonToken.END_DOCUMENT){ //2013.5.31 容错处理，避免因JSON数据问题导致死循环
                    count = 0;
                }
            } while (count != 0);
        } finally {
